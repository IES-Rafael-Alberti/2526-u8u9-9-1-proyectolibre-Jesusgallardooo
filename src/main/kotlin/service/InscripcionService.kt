...
/**
 * Servicio que gestiona la lógica de negocio de las inscripciones.
 * Persiste simultáneamente en CSV y en H2.
 * Valida que el socio esté activo, no esté ya inscrito y haya plazas disponibles.
 */
class InscripcionService(
    ...
    fun listarInscripcionesPorSocio(socioId: Long): List<Inscripcion> = csvRepo.findAll().filter { it.socioId == socioId }
    fun listarInscripcionesPorActividad(actividadId: Long): List<Inscripcion> = csvRepo.findAll().filter { it.actividadId == actividadId }
